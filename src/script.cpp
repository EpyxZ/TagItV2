...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_SMALLOGDATA              : return "OP_SMALLOGDATA";
...
bool IsDERSignature(const valtype &vchSig, bool fWithHashType, bool fCheckLow) {
    ...
    if (fWithHashType) {
        ...
        if (nHashType < SIGHASH_ALLOG || nHashType > SIGHASH_SINGLE)
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        mTemplates.insert(make_pair(TX_MULTISIG, CScript() << OP_SMALLOGINTEGER << OP_PUBKEYS << OP_SMALLOGINTEGER << OP_CHECKMULTISIG));

        // Empty, provably prunable, data-carrying output
        mTemplates.insert(make_pair(TX_NULL_DATA, CScript() << OP_RETURN << OP_SMALLOGDATA));
    }
    ...
    BOOST_FOREACH(const PAIRTYPE(txnouttype, CScript)& tplate, mTemplates)
    {
        ...
        for ( ; ; )
        {
            ...
            else if (opcode2 == OP_SMALLOGINTEGER)
            ...
            else if (opcode2 == OP_SMALLOGDATA)
            {
...
isminetype IsMine(const CKeyStore &keystore, const CScript& scriptPubKey)
{
    ...
    case TX_MULTISIG:
    {
        // Only consider transactions "mine" if we own ALLOG the
