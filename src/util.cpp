...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TagIt
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TagIt
    // Mac: ~/Library/Application Support/TagIt
    // Unix: ~/.TagIt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TagIt";
#else
    fs::path pathRet;
...
#endif
    pConf << "#Head to tagit.site for more information\nrpcuser=TagIt\nrpcpassword="
            + randomStrGen(15)
            + "\nrpcport=2810"
            + "\ndaemon=1"
            + "\nserver=1"
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TagIt.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tagitd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong TagIt will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("TagIt"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
