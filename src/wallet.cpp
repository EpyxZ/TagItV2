...
CMalleableKeyView CWallet::GenerateNewMalleableKey()
{
    ...
    // Compressed public keys were introduced in version 0.6.0
    SetMinVersion(FEATURE_MALLOGKEY);
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    vMasterKey.resize(WALLOGET_CRYPTO_KEY_SIZE);
    RAND_bytes(&vMasterKey[0], WALLOGET_CRYPTO_KEY_SIZE);

    CMasterKey kMasterKey;

    RandAddSeedPerfmon();
    kMasterKey.vchSalt.resize(WALLOGET_CRYPTO_SALT_SIZE);
    RAND_bytes(&kMasterKey.vchSalt[0], WALLOGET_CRYPTO_SALT_SIZE);
...

    {
        ...
        SetMinVersion(FEATURE_WALLOGETCRYPT, pwalletdbEncryption, true);
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sALLOG %s\n", FormatMoney(wtx.GetCredit(MINE_ALLOG)).c_str(), wtx.GetHash().ToString().c_str());
...
{
    return (GetDebit(tx, MINE_ALLOG) > 0);
}
...
bool CWalletTx::IsTrusted() const
{
    ...
    if (nDepth < 0)
        return false;
    if (!fConfChange || !IsFromMe(MINE_ALLOG)) // using wtx's cached debit
        return false;
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %sALLOG %s\n", FormatMoney(wtx.GetCredit(MINE_ALLOG)).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
int64_t CWallet::GetStake() const
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0 && pcoin->GetDepthInMainChain() > 0)
            nTotal += CWallet::GetCredit(*pcoin, MINE_ALLOG);
    }
    return nTotal;
}
...
bool CWallet::SelectCoinsMinConf(int64_t nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
        ...
        if (output.nDepth < (pcoin->IsFromMe(MINE_ALLOG) ? nConfMine : nConfTheirs))
            continue;
...
bool CWallet::CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nGenerationTime, uint32_t nBits, CTransaction &txNew, CKey& key)
{
    ...
    if (GetWeight((int64_t)wtx.nTime, (int64_t)nGenerationTime) == nStakeMaxAge)
    {
        ...
        //
        // * Value is higher than 0.01 ALLOG;
...
{
    {
        ...
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            ...
            printf("    PoS: %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit(MINE_ALLOG));
        }
        else if (mapWallet.count(block.vtx[0].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[0].GetHash()];
            printf("    PoW:  %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit(MINE_ALLOG));
        }
    }
    printf("\n");
}
...
std::map<CBitcoinAddress, int64_t> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            if (nDepth < (pcoin->IsFromMe(MINE_ALLOG) ? 0 : 1))
                continue;
