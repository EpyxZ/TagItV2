...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
        ...
        int64_t nCredit = wtx.GetCredit(MINE_ALLOG);
        int64_t nDebit = wtx.GetDebit(MINE_ALLOG);
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                nUnmatured += wallet->GetCredit(txout, MINE_ALLOG);
            ...
            else
            {
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, -wallet->GetDebit(txin, MINE_ALLOG)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, wallet->GetCredit(txout, MINE_ALLOG)) + "<br>";
            }
        }
