...
{
    resize(850, 550);
    setWindowTitle(tr("Tagit") + " - " + tr("(IT) Wallet"));
    qApp->setStyleSheet("QMainWindow { background-image:url(:images/bkg);border:none; }"); // Added Background static background 11-9-16 allion.us
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a TagIt address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About TagIt"), this);
    aboutAction->setStatusTip(tr("Show information about TagIt"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for TagIt"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your TagIt addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified TagIt addresses"));
    secondAuthAction = new QAction(QIcon(":/icons/key"), tr("Second &auth..."), this);
    secondAuthAction->setStatusTip(tr("Second auth with your TagIt addresses"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("TagIt client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("TagIt client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to TagIt network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid TagIt address or malformed URI parameters."));
    }
