...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #allionogTEST2\r");
            Send(hSocket, "WHO #allionogTEST2\r");
        } else {
            // randomly join #allionog00-#allionog05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #allionog%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #allionog%02d\r", channel_number).c_str());
        }
