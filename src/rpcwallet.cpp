...
void EnsureWalletIsUnlocked()
{
    if (pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLOGET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
    if (fWalletUnlockMintOnly)
        throw JSONRPCError(RPC_WALLOGET_UNLOCK_NEEDED, "Error: Wallet unlocked for block minting only.");
}
...
string AccountFromValue(const Value& value)
{
    ...
    if (strAccount == "*")
        throw JSONRPCError(RPC_WALLOGET_INVALID_ACCOUNT_NAME, "Invalid account name");
    return strAccount;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new TagIt address for receiving payments.  "
    ...
    if (!pwalletMain->GetKeyFromPool(newKey, false))
        throw JSONRPCError(RPC_WALLOGET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current TagIt address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <allionogaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TagIt address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <allionogaddress> <amount> [comment] [comment-to]\n"
    ...
    if (!strError.empty())
        throw JSONRPCError(RPC_WALLOGET_ERROR, strError);
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <allionogaddress> <message>\n"
    ...
    if (!pwalletMain->GetKey(keyID, key))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Private key not available");
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <allionogaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <allionogaddress> [minconf=1]\n"
            "Returns the total amount received by <allionogaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toallionogaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest " + FormatMoney(nMinimumInputValue)
            + HelpRequiringPassphrase());
    ...
    if (nAmount > nBalance)
        throw JSONRPCError(RPC_WALLOGET_INSUFFICIENT_FUNDS, "Account has insufficient funds");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid TagIt address: ")+s.name_);
    ...
    if (!fCreated)
    {
        ...
        if (totalAmount + nFeeRequired > nTotal - nWatchOnly)
            throw JSONRPCError(RPC_WALLOGET_INSUFFICIENT_FUNDS, "Insufficient funds");
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Transaction creation failed");
    }
    if (!pwalletMain->CommitTransaction(wtx, keyChange))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Transaction commit failed");
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a TagIt address or hex-encoded public key\n"
...
Value backupwallet(const Array& params, bool fHelp)
{
    ...
    if (!BackupWallet(*pwalletMain, strDest))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Error: Wallet backup failed!");
...
Value keypoolrefill(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->GetKeyPoolSize() < nSize)
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Error refreshing keypool.");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLOGET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassphrase was called.");

    if (!pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLOGET_ALREADY_UNLOCKED, "Error: Wallet is already unlocked, use walletlock first if need to change unlock settings.");
    ...
    if (strWalletPass.length() > 0)
    {
        if (!pwalletMain->Unlock(strWalletPass))
            throw JSONRPCError(RPC_WALLOGET_PASSPHRASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
    }
    else
        throw runtime_error(
...
Value walletpassphrasechange(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLOGET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassphrasechange was called.");
...
Value walletlock(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLOGET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletlock was called.");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLOGET_WRONG_ENC_STATE, "Error: running with an encrypted wallet, but encryptwallet was called.");
    ...
    if (!pwalletMain->EncryptWallet(strWalletPass))
        throw JSONRPCError(RPC_WALLOGET_ENCRYPTION_FAILED, "Error: Failed to encrypt the wallet.");

    // BDB seems to have a bad habit of writing old data into
    // slack space in .dat files; that is bad if the old data is
    // unencrypted private keys. So:
    StartShutdown();
    return "wallet encrypted; TagIt server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <allionogaddress>\n"
            "Return information about <allionogaddress>.");
