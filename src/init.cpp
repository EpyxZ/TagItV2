...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("TagIt version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  tagitd [options]                     " + "\n" +
                  "  tagitd [options] <command> [params]  " + _("Send command to -server or tagitd") + "\n" +
                  "  tagitd [options] help                " + _("List commands") + "\n" +
                  "  tagitd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "allionog:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("TagIt"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("TagIt"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: allionog.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: tagitd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 19621 or testnet: NA)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 19611 or testnet: NA)") + "\n" +
...
bool AppInit2()
{
    ...
    _set_abort_behavior(0, _WRITE_ABORT_MSG | _CALLOG_REPORTFAULT);
#endif
#ifdef WIN32
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    SoftSetArg("-addnode", "ALLOG.allion.us");
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  TagIt is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "TagIt server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("TagIt"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of TagIt") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart TagIt to complete") << "\n";
