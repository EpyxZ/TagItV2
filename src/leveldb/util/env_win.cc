...
#define DISALLOGOW_COPY_AND_ASSIGN(TypeName) \
...
private:
    ...
    ::HANDLE _hFile;
    DISALLOGOW_COPY_AND_ASSIGN(Win32SequentialFile);
};
...
private:
    ...
    DISALLOGOW_COPY_AND_ASSIGN(Win32RandomAccessFile);
};
...
private:
    ...
    bool _MapNewRegion();
    DISALLOGOW_COPY_AND_ASSIGN(Win32MapFile);
    BOOL _Init(LPCWSTR Path);
};
...
private:
    ...
    HANDLE _hFile;
    std::string _filename;
    DISALLOGOW_COPY_AND_ASSIGN(Win32FileLock);
};
...
private:
    ...
    DISALLOGOW_COPY_AND_ASSIGN(Win32Logger);
};
...
std::string GetLastErrSz()
{
    LPWSTR lpMsgBuf;
    FormatMessageW( 
        FORMAT_MESSAGE_ALLOGOCATE_BUFFER | 
