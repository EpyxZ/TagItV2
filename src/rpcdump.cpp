...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <allionogprivkey> [label] [rescan=true]\n"
    ...
    if (fWalletUnlockMintOnly) // ppcoin: no importprivkey in mint-only mode
        throw JSONRPCError(RPC_WALLOGET_UNLOCK_NEEDED, "Wallet is unlocked for minting only.");
    ...
    CBitcoinAddress addr = CBitcoinAddress(keyid);
    {
        ...
        if (!pwalletMain->AddKey(key))
            throw JSONRPCError(RPC_WALLOGET_ERROR, "Error adding key to wallet");
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TagIt address or script");
...

    {
        ...
        if (::IsMine(*pwalletMain, script) == MINE_SPENDABLE)
            throw JSONRPCError(RPC_WALLOGET_ERROR, "The wallet already contains the private key for this address or script");
        ...
        if (!pwalletMain->AddWatchOnly(script))
            throw JSONRPCError(RPC_WALLOGET_ERROR, "Error adding address to wallet");
...
Value removeaddress(const Array& params, bool fHelp)
{
    ...
    if (::IsMine(*pwalletMain, script) == MINE_SPENDABLE)
        throw JSONRPCError(RPC_WALLOGET_ERROR, "The wallet contains the private key for this address or script - can't remove it");

    if (!pwalletMain->HaveWatchOnly(script))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "The wallet does not contain this address or script");
    ...
    if (!pwalletMain->RemoveWatchOnly(script))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Error removing address from wallet");
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    if(!ImportWallet(pwalletMain, params[0].get_str().c_str()))
       throw JSONRPCError(RPC_WALLOGET_ERROR, "Error adding some keys to wallet");
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <allionogaddress>\n"
            "Reveals the private key corresponding to <allionogaddress>.");
    ...
    if (!pwalletMain->GetSecret(keyID, vchSecret, fCompressed))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Private key for address " + strAddress + " is not known");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <allionogaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <allionogaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
    ...
    if (!pwalletMain->GetPEM(keyID, params[1].get_str(), strPassKey))
        throw JSONRPCError(RPC_WALLOGET_ERROR, "Error dumping key pair to file");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    if(!DumpWallet(pwalletMain, params[0].get_str().c_str() ))
      throw JSONRPCError(RPC_WALLOGET_ERROR, "Error dumping wallet keys to file");
